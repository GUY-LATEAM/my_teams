...
static int do_list_command(server_t *server, user_t *user, char **context)
{
    if (server == NULL || context == NULL) {
        return EXIT_FAILURE;
    }
    if (strcmp(context[0], UNDEFINED) == 0) {
        return list_team(server, user, context);
    }
    if (strcmp(context[1], UNDEFINED) == 0) {
        return list_channel(server, user, context);
    }
    if (strcmp(context[2], UNDEFINED) == 0) {
        return list_thread(server, user, context);
    } else {
        return list_reply(server, user, context);
    }
}
...
__attribute__((unused)) circular_buffer_t *write_buffer)
{
    ...
    if (is_a_good_context(context) == false) {
        ...
        return do_list_command(server, user_data, context);
    }
    return EXIT_SUCCESS;
}
